#!/usr/bin/env python3
"""
IRS Form 990/990-EZ Balance Sheet Parser
=========================================
CUSTOMIZED VERSION - Uses pre-filtered CSV files with URLs
...
Input Files:
    - tax990.csv:   CSV with columns [FILEREIN, TAXYEAR, URL] for Form 990
    - tax990EZ.csv: CSV with columns [FILEREIN, TAXYEAR, URL] for Form 990-EZ

Usage:
    # Test with small sample (10 files from each form type)
    python irs990_balance_sheet_parser_custom.py --sample 10
    
    # Full run with all URLs from your CSV files
    python irs990_balance_sheet_parser_custom.py --file990 tax990.csv --file990ez tax990EZ.csv
    
    # Specify output location
    python irs990_balance_sheet_parser_custom.py --output ./my_output/
...
class Config:
    ...
    # Default input file paths (can be overridden via command line)
    # Use ~ for home directory, will be expanded automatically
    DEFAULT_990_FILE = "tax990.csv"
    DEFAULT_990EZ_FILE = "tax990EZ.csv"
    
    # Output directory for downloaded files and results
    OUTPUT_DIR = Path("./irs990_output")
...
XPATH_990EZ = {
    # Line 23: Land and buildings
    'land_buildings': [
        './/LandAndBuildingsGrp/EOYAmt',               # 2013+ format
        './/LandAndBuildings/EOY',                      # Older format
        './/LandAndBuildingsAmt',                       # Variant
    ],
    
    # Line 24: Other assets
    'other_assets': [
        './/OtherAssetsTotalGrp/EOYAmt',               # 2013+ format
        './/OtherAssetsTotal/EOY',                      # Older format
        './/OtherAssetsTotalAmt',                       # Variant
    ],
    
    # Line 22: Cash, savings, investments (combined in 990-EZ)
    'cash_savings_investments': [
        './/CashSavingsAndInvestmentsGrp/EOYAmt',
        './/CashSavingsAndInvestments/EOY',
        './/CashSavingsAndInvestmentsAmt',
    ],
    
    # Line 25: Total assets (for validation)
    'total_assets': [
        './/Form990TotalAssetsGrp/EOYAmt',
        './/TotalAssetsEOYAmt',
        './/TotalAssets/EOY',
        './/TotalAssetsEOY',
    ],
...
def setup_logging(verbose: bool = False) -> logging.Logger:
    ...
    # Create logger
    logger = logging.getLogger('irs990_parser')
    ...
    # File handler
    log_file = Config.LOGS_DIR / f"parser_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def load_url_csv(file_path: str, form_type: str) -> List[Dict]:
    """
    ...
    Args:
        ...
        form_type: '990' or '990EZ' (added to each record)
    ...
    logger.info(f"Loading {form_type} URLs from: {file_path}")
    ...
    logger.info(f"Loaded {len(entries):,} {form_type} entries")
    return entries
...
def download_xml(entry: Dict) -> Optional[Path]:
    ...
    # Create local filename: {EIN}_{TaxYear}_{FormType}.xml
    filename = f"{ein}_{tax_year}_{form_type}.xml"
...
def parse_990ez(xml_path: Path, entry: Dict) -> Optional[Dict]:
    """
    Parse balance sheet fields from a Form 990-EZ XML file.
    ...
    try:
        ...
        result = {
            ...
            # ===== PRIMARY FIELDS (NOT in datamart) =====
            'land_buildings_eoy': extract_value(root, XPATH_990EZ['land_buildings']),
            'other_assets_eoy': extract_value(root, XPATH_990EZ['other_assets']),
            
            # ===== VALIDATION FIELDS =====
            'cash_savings_investments_eoy': extract_value(root, XPATH_990EZ['cash_savings_investments']),
            'total_assets_eoy': extract_value(root, XPATH_990EZ['total_assets']),
            
            # Header validation
            'xml_ein': header['ein'],
            'xml_tax_year': header['tax_year'],
            
            # Placeholders for consistency with 990 output
            # (990-EZ doesn't break out investments separately)
            'land_buildings_equipment_eoy': None,  # Use land_buildings_eoy instead
            'investments_pub_traded_eoy': None,
            'investments_other_sec_eoy': None,
            'investments_program_related_eoy': None,
        }
...
def parse_all_files(entries: List[Dict]) -> List[Dict]:
    ...
    for i, entry in enumerate(entries):
        ...
        if entry['form_type'] == '990':
            result = parse_990(local_path, entry)
        elif entry['form_type'] == '990EZ':
            result = parse_990ez(local_path, entry)
        else:
            logger.warning(f"Unknown form type: {entry['form_type']}")
            result = None
...
def save_results(results: List[Dict], output_path: Optional[Path] = None) -> Path:
    ...
    if output_path is None:
        ...
        output_path = Config.RESULTS_DIR / f"balance_sheet_data_{timestamp}.csv"
...
def print_summary(results: List[Dict]):
    ...
    # Count non-null values for primary fields
    primary_fields_990 = ['land_buildings_equipment_eoy', 'investments_pub_traded_eoy', 
                          'investments_other_sec_eoy', 'investments_program_related_eoy']
    primary_fields_ez = ['land_buildings_eoy', 'other_assets_eoy']
    ...
    # 990 fields
    count_990 = form_counts.get('990', 0)
    if count_990 > 0:
        print(f"\n  Form 990 ({count_990:,} records):")
        for field in primary_fields_990:
            non_null = sum(1 for r in results if r.get('form_type') == '990' and r.get(field))
            pct = (non_null / count_990) * 100 if count_990 > 0 else 0
            print(f"    {field}: {non_null:,} ({pct:.1f}%)")
    
    # 990-EZ fields
    count_ez = form_counts.get('990EZ', 0)
    if count_ez > 0:
        print(f"\n  Form 990-EZ ({count_ez:,} records):")
        for field in primary_fields_ez:
            non_null = sum(1 for r in results if r.get('form_type') == '990EZ' and r.get(field))
            pct = (non_null / count_ez) * 100 if count_ez > 0 else 0
            print(f"    {field}: {non_null:,} ({pct:.1f}%)")
...
def run_pipeline(
    ...
    Args:
        file_990: Path to CSV with Form 990 URLs
        file_990ez: Path to CSV with Form 990-EZ URLs
        sample_size: If set, sample this many records from EACH file (for testing)
    ...
    print("\n" + "=" * 60)
    print("IRS 990 Balance Sheet Parser (Custom CSV Version)")
    print("=" * 60)
    
    # Step 1: Load URL files
    logger.info("\n--- Step 1: Loading URL Files ---")
    
    entries_990 = load_url_csv(file_990, '990')
    entries_990ez = load_url_csv(file_990ez, '990EZ')
    
    if not entries_990 and not entries_990ez:
        logger.error("No entries loaded from either file!")
        return None
    
    # Step 2: Sample if requested (for testing)
    if sample_size:
        logger.info(f"\n--- Step 2: Sampling {sample_size} entries from each file ---")
        entries_990 = sample_entries(entries_990, sample_size)
        entries_990ez = sample_entries(entries_990ez, sample_size)
        logger.info(f"Sampled: {len(entries_990)} Form 990, {len(entries_990ez)} Form 990-EZ")
    
    # Combine entries
    all_entries = entries_990 + entries_990ez
    ...
    if skip_download:
        ...
        # Add local_path to entries from cache
        for entry in all_entries:
            filename = f"{entry['ein']}_{entry['tax_year']}_{entry['form_type']}.xml"
            local_path = Config.XML_CACHE_DIR / filename
            entry['local_path'] = local_path if local_path.exists() else None
        
        cached_count = sum(1 for e in all_entries if e.get('local_path'))
        logger.info(f"Found {cached_count:,} cached files")
    else:
        all_entries = download_xml_batch(all_entries)
    
    # Step 4: Parse XMLs
    logger.info("\n--- Step 4: Parsing XML Files ---")
    results = parse_all_files(all_entries)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Extract balance sheet fields from IRS 990 XML files using your CSV URL lists',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Test with small sample (10 from each file)
    python irs990_balance_sheet_parser_custom.py --sample 10
    
    # Full run with default file locations
    python irs990_balance_sheet_parser_custom.py
    
    # Specify file locations (Mac paths)
    python irs990_balance_sheet_parser_custom.py \\
        --file990 "~/Documents/job search 2025/tax990.csv" \\
        --file990ez "~/Documents/job search 2025/tax990EZ.csv"
    
    # Skip download (parse cached files only)
    python irs990_balance_sheet_parser_custom.py --skip-download
    
    # Custom output directory
    python irs990_balance_sheet_parser_custom.py --output ~/Desktop/990_output/
        """
    )
    ...
    parser.add_argument(
        '--file990',
        type=str,
        default=Config.DEFAULT_990_FILE,
        help=f'Path to CSV with Form 990 URLs (default: {Config.DEFAULT_990_FILE})'
    )
    
    parser.add_argument(
        '--file990ez',
        type=str,
        default=Config.DEFAULT_990EZ_FILE,
        help=f'Path to CSV with Form 990-EZ URLs (default: {Config.DEFAULT_990EZ_FILE})'
    )
    
    parser.add_argument(
        '--sample',
        type=int,
        help='Sample size for testing (e.g., 10 takes 10 from each file)'
    )
    ...
    output_path = run_pipeline(
        file_990=args.file990,
        file_990ez=args.file990ez,
